...
from reprodbench.pipeline.judge_patched_functional_requirements import (
    PatchedFunctionalRequirementsJudge,
)
from reprodbench.pipeline.patched_functional_requirements import (
    PatchedFunctionalRequirementsExtractor,
    PatchedFunctionalRequirementsRefiner,
)
...
load_dotenv()
logger = setup_logger("reprodbench.patched_functional_requirements")
...
def run_for_model(MODEL_NAME: str, DATA_PATH: str):
    ...
    RESULTS_PATH = (
        RESULTS_DIR
        / f"patched_functional_requirements_{MODEL_NAME}_{JUDGE_MODEL_NAME}_{RUN_TS}.csv"
    )
    ...
    extractor = PatchedFunctionalRequirementsExtractor(
    ...
    refiner = PatchedFunctionalRequirementsRefiner(
        prompt_dir=GEN_PROMPT_DIR,
        provider="ollama",
        model=MODEL_NAME,
        temperature=1.0,
    ...
    judge = PatchedFunctionalRequirementsJudge(
    ...
    for idx, row in dataset.iterrows():
        ...
        for gen_attempt in range(1, MAX_GENERATOR_RETRIES + 1):
            try:
                ...
                log_step(
                    logger,
                    f"[{MODEL_NAME}] Artifact: {artifact.functional_requirements}",
                )
                patched_fr = artifact.functional_requirements
        ...
        for cycle in range(1, MAX_CYCLES + 1):
            ...
            for judge_attempt in range(1, MAX_JUDGE_RETRIES + 1):
                try:
                    judge_result = judge.judge(
                        question=question,
                        answer=answer,
                        functional_requirements=patched_fr,
                    )
                    log_step(
            ...
            patched_fr = refined_artifact.functional_requirements
        ...
        print("\n" + "-" * 80)
        print("FINAL PATCHED FUNCTIONAL REQUIREMENTS")
        print("-" * 80)
        ...
        print("\nPatched Functional Requirements:")
        print(patched_fr)
